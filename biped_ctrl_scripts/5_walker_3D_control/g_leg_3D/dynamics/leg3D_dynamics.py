...
Ihx, Ihy, Ihz = sp.symbols('Ihx, Ihy, Ihz', real=True)
...
u = sp.Matrix([0,1,0])
omega1_in_1 = omega1 * u
omega_1_to_0 = omega0 + R_1_to_0 @ omega1_in_1

u = sp.Matrix([0,1,0])
omega2_in_2 = omega2 * u
omega_2_to_0 = omega0 + R_1_to_0 @ R_2_to_1 @ omega2_in_2
...
I_hip = sp.diag(Ihx, Ihy, Ihz)

T = 0.5 * (m_foot * v_foot.dot(v_foot) + m_calf * v_calf_com.dot(v_calf_com) + m_knee * v_knee.dot(v_knee) + m_thigh * v_thigh_com.dot(v_thigh_com) + m_hip * v_hip_com.dot(v_hip_com) + omega_1_to_0.dot(I_hip @ omega_1_to_0) + omega_1_to_0.dot(I_thigh @ omega_1_to_0) + omega_2_to_0.dot(I_calf @ omega_2_to_0))
           
V = m_foot * g * zp_foot + m_calf * g * zp_calf_com + m_knee * g * zp_knee + m_thigh * g * zp_thigh_com + m_hip * g * zp_hip_com
...
EOM_vec = sp.Matrix([EOM[i] for i in range(3)])
# print(EOM_vec)

# Compute the system's M_ss matrix and b_ss vector
M = EOM_vec.jacobian(qddot)
...
N = EOM_vec.subs([(alpha0,0), (alpha1,0), (alpha2,0)])
# M(q) q'' + B(q,q') = tau
