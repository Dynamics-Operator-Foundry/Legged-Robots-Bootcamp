...
def main():
    ...
    print("GOT LAGRAGIAN")
    ##################################
    dof = 14 # dimension of q
    
    EOM = []
    for i in range(dof):
        dLdqdot = sp.diff(L, qdot[i], )
        ddt_dLdqdot = sum([sp.diff(dLdqdot, q[j], ) * qdot[j] + sp.diff(dLdqdot, qdot[j], ) * qddot[j] for j in range(dof)])
    ...
    EOM_vec = sp.Matrix([EOM[i] for i in range(dof)])
    print(EOM_vec.shape)
    
    folder_des = os.path.abspath(os.path.join(os.path.dirname(__file__), './compiled_funcs'))
    with open(folder_des + "/EOM_equations.pkl", "wb") as file:
        pickle.dump(EOM_vec, file)
        
    # Loading the saved equations
    with open(folder_des + "/EOM_equations.pkl", "rb") as file:
        EOM_lala = pickle.load(file)
    print(EOM_lala.shape)
