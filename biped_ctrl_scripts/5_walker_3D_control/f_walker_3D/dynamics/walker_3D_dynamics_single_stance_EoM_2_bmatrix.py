...
def main():
    ...
    B_ss = -EOM_vec
    print(B_ss.shape)
    with open(folder_des + "/B_matrix.pkl", "wb") as file:
        pickle.dump(B_ss, file)
        
    with open(folder_des + "/B_matrix.pkl", "rb") as file:
        B_mat = pickle.load(file)
    print(B_mat.shape)
    print("M Mat loaded successfully.")
