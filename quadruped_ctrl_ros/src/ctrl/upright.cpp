...
void ctrl_server::upwright_ctrl()
{
    if(!swing_track_start)
    {
        set_upwright_ctrl();
        set_upwright_ctrl_gain();
...
void ctrl_server::set_upwright_ctrl()
{
    p_swing_target <<
    ...
    std::cout<<q_swing_target<<std::endl;

    for (int i = 0; i < DoF; i++)
        cmdSet.motorCmd[i].q = q_state[i];
    
    Kp = Eigen::Vector3d(20, 20, 50).asDiagonal();
    Kd = Eigen::Vector3d( 5,  5, 20).asDiagonal();
}

void ctrl_server::set_upwright_ctrl_gain()
{
    for(int leg_i = 0;;)
    {
...
void ctrl_server::upwright_ctrl_reset()
{
